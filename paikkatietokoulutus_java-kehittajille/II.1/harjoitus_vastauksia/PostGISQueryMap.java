...
public class PostGISQueryMap {
    ...
    private void doFilterFactory2Query() throws Exception {
        ...
        String geomName = "geom";//schema.getGeometryDescriptor().getLocalName();

        //CoordinateReferenceSystem crs = DefaultGeographicCRS.WGS84;
        CoordinateReferenceSystem crs = CRS.decode("EPSG:4326");
        ReferencedEnvelope envelope = new ReferencedEnvelope(20.86, 30.34, 65.01, 70.10, crs);
        ...
        Intersects intersectsFilter = filterFactory2.intersects(filterFactory2.property(geomName), filterFactory2.literal(polygon));
    ...
    private void doCQLBBOXQuery() throws Exception {
        SimpleFeatureCollection simpleFeatureCollection = getFeatures("BBOX(geom, 20.86, 65.01, 30.34, 70.10)"); // EPSG:4326, Pohjois-Suomi
        Style style = SLD.createSimpleStyle(sourceRead.getSchema());
    ...
    private void doCQLDISJOINTQuery() throws Exception {
        SimpleFeatureCollection simpleFeatureCollection = getFeatures("DISJOINT(geom, POLYGON((20.86 65.01, 20.86 70.10, 30.34 70.10, 30.34 65.01, 20.86 65.01)))"); // EPSG:4326, ei Pohjois-Suomi
        Style style = SLD.createSimpleStyle(sourceRead.getSchema());
    ...
    private void addFeatures() throws Exception {
        ...
        SimpleFeatureCollection sourceFeatures = getFeatures("fclass='city' OR fclass='national_capital'");
    ...
    private SimpleFeatureCollection getFeatures(String filterText) throws Exception {
        Filter filter = CQL.toFilter(filterText);
        SimpleFeatureCollection features = sourceRead.getFeatures(filter);
        return features;
    }
